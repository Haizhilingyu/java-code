...
public class RegexUtil {
     ...
     * @author jiqinlin
     */
    public final static boolean isMac(String str) {
     ...
     */
    public static boolean isIpAddressStr(String address, boolean isIpv4) {
        if (isBlank(address)) {
            return false;
        }
        String rex;
        if (isIpv4) {
            ...
            rex = "([1-9]|[1-9]\\d|1\\d{2}|2[0-4]\\d|25[0-5])" +
                    "(\\.(\\d|[1-9]\\d|1\\d{2}|2[0-4]\\d|25[0-5])){3}";
        } else {
            rex = "^((([0-9A-Fa-f]{1,4}:){7}[0-9A-Fa-f]{1,4})" +
                    "|(([0-9A-Fa-f]{1,4}:){1,7}:)" +
